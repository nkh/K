...
fn buffer_new_default_cells() {
    let b = vrunner::vtty::buffer::Buffer::new(10, 5, 100);
    for row in &b.rows {
        for cell in row {
            assert_eq!(cell.ch, ' ');
        }
...
fn buffer_resize_grow() {
    use vrunner::vtty::cell::Cell;
    let mut b = vrunner::vtty::buffer::Buffer::new(5, 3, 100);
...
fn buffer_insert_cells_shifts_right() {
    use vrunner::vtty::cell::Cell;
    let mut b = vrunner::vtty::buffer::Buffer::new(10, 1, 100);
    b.set(0, 0, Cell::new('A'));
...
fn buffer_diff_single_cell_change() {
    use vrunner::vtty::cell::Cell;
    let mut a = vrunner::vtty::buffer::Buffer::new(10, 5, 100);
    let b = vrunner::vtty::buffer::Buffer::new(10, 5, 100);
...
fn cell_default_is_space() {
    let c = vrunner::vtty::cell::Cell::default();
    assert_eq!(c.ch, ' ');
    assert_eq!(c.width, 1);
    assert!(!c.bold);
    assert!(!c.italic);
...
fn cell_new_with_char() {
    let c = vrunner::vtty::cell::Cell::new('X');
    assert_eq!(c.ch, 'X');
    assert_eq!(c.width, 1);
}
...
fn color_256_grayscale_ramp() {
    let first = vrunner::vtty::color::color_256_to_rgb(232);
    let last = vrunner::vtty::color::color_256_to_rgb(255);
    assert!(first[0] < last[0]); // grayscale ramp increases
}

#[test]
fn color_palette_new_has_256_entries() {
    let p = vrunner::vtty::color::ColorPalette::new();
...
fn color_palette_set_and_resolve() {
    let mut p = vrunner::vtty::color::ColorPalette::new();
    p.set(10, [100, 200, 50]);
...
fn make_emulator(rows: u16, cols: u16) -> vrunner::vtty::emulator::VttyEmulator {
    vrunner::vtty::emulator::VttyEmulator::new(rows, cols, 1000)
}
...
fn config_serialize_roundtrip() {
    ...
    let cfg2: vrunner::config::schema::Config = serde_json::from_str(&json).unwrap();
...
fn config_merge_local_overrides_global() {
    let mut global = vrunner::config::schema::Config::default();
    global.server.port = 9090;
    let mut local = vrunner::config::schema::Config::default();
    local.server.port = 8080;
    let merged = vrunner::config::merge::merge_configs(global, local);
    assert_eq!(merged.server.port, 8080);
}

#[test]
fn config_merge_handles_keeps_global_when_local_empty() {
    use vrunner::config::schema::HandleConfig;
    let mut global = vrunner::config::schema::Config::default();
    ...
    let local = vrunner::config::schema::Config::default();
    let merged = vrunner::config::merge::merge_configs(global, local);
...
fn config_apply_profile_overrides_base() {
    use vrunner::config::schema::PartialConfig;
    let mut base = vrunner::config::schema::Config::default();
    base.server.port = 9090;
    let mut profile = PartialConfig::default();
    let mut server = vrunner::config::schema::ServerConfig::default();
    server.port = 3000;
    profile.server = Some(server);
    let result = vrunner::config::merge::apply_profile(base, &profile);
    assert_eq!(result.server.port, 3000);
}

#[test]
fn config_apply_profile_none_keeps_base() {
    let base = vrunner::config::schema::Config::default();
    let profile = vrunner::config::schema::PartialConfig::default();
    let result = vrunner::config::merge::apply_profile(base, &profile);
...
fn config_environment_variables() {
    ...
    let env = vrunner::config::schema::EnvironmentConfig {
...
fn error_command_not_found_display() {
    let err = vrunner::process::error::ProcessError::CommandNotFound("xyz".into());
    let msg = format!("{}", err);
...
fn error_spawn_failed_display() {
    let err = vrunner::process::error::ProcessError::SpawnFailed { cmd: "bash".into() };
    let msg = format!("{}", err);
...
fn error_from_io_error() {
    ...
    let err: vrunner::process::error::ProcessError = io_err.into();
    assert!(matches!(err, vrunner::process::error::ProcessError::Io(_)));
}

#[test]
fn error_io_has_source() {
    let io_err = std::io::Error::new(std::io::ErrorKind::PermissionDenied, "denied");
    let err = vrunner::process::error::ProcessError::Io(io_err);
    assert!(std::error::Error::source(&err).is_some());
}

#[test]
fn error_non_io_no_source() {
    let err = vrunner::process::error::ProcessError::ChannelClosed("c1".into());
...
fn error_is_send_sync() {
    ...
    assert_send_sync::<vrunner::process::error::ProcessError>();
}
...
fn error_sink_already_exists_display() {
    let err = vrunner::process::error::ProcessError::SinkAlreadyExists {
        name: "out".into(),
...
fn error_snapshot_not_found_display() {
    let err = vrunner::process::error::ProcessError::SnapshotNotFound {
        name: "snap".into(),
        command_id: "c1".into(),
    };
    let msg = format!("{}", err);
...
fn handle_registry_new_empty() {
    let reg = vrunner::handles::registry::HandleRegistry::new();
    assert!(reg.list().is_empty());
}

#[test]
fn handle_registry_default_trait() {
    let reg = vrunner::handles::registry::HandleRegistry::default();
    assert!(reg.list().is_empty());
}
...
async fn file_sink_write_and_read() {
    ...
    let mut sink = vrunner::handles::file_sink::FileSink::new(path.to_str().unwrap()).unwrap();
...
async fn file_sink_append() {
    ...
    {
        let mut s2 = vrunner::handles::file_sink::FileSink::new(path.to_str().unwrap()).unwrap();
        s2.write(b"second\n").await;
        s2.flush().await;
    }
...
fn logger_disabled_no_output() {
    let logger = vrunner::logging::command_log::CommandLogger::new(false, None).unwrap();
...
fn logger_enabled_stores_in_memory() {
    let logger = vrunner::logging::command_log::CommandLogger::new(true, None).unwrap();
...
fn renderer_to_html_basic() {
    use vrunner::vtty::cell::Cell;
    let mut buf = vrunner::vtty::buffer::Buffer::new(10, 2, 100);
...
fn rate_limiter_first_call_succeeds() {
    let mut limiter = vrunner::vtty::rate_limiter::RateLimiter::new(10);
    assert!(limiter.allow()); // first call always succeeds
}

#[test]
fn rate_limiter_disabled_always_allows() {
    let mut limiter = vrunner::vtty::rate_limiter::RateLimiter::disabled();
    for _ in 0..100 {
...
fn rate_limiter_max_rate_config() {
    let limiter = vrunner::vtty::rate_limiter::RateLimiter::new(30);
...
fn instance_info_serialization_roundtrip() {
    let info = vrunner::instance::info::InstanceInfo {
        pid: 12345,
        port: 9090,
        bind: "0.0.0.0".into(),
    ...
    let info2: vrunner::instance::info::InstanceInfo = serde_json::from_str(&json).unwrap();
...
fn vtty_output_new_has_no_sinks() {
    let output = vrunner::vtty::sink::VttyOutput::new();
...
fn vtty_output_with_sinks() {
    use std::sync::Arc;
    let sink = Arc::new(vrunner::vtty::sink::InMemoryVttySink::new());
    let output = vrunner::vtty::sink::VttyOutput::with_sinks(vec![sink.clone()]);
    assert_eq!(output.sink_count(), 1);
}

#[test]
fn vtty_in_memory_sink_initially_empty() {
    let sink = vrunner::vtty::sink::InMemoryVttySink::new();
...
fn merge_command_env_empty_overrides() {
    use std::collections::HashMap;
    let config_env = vrunner::config::schema::EnvironmentConfig {
        variables: HashMap::from([("A".into(), "1".into())]),
    };
    let merged = vrunner::config::merge::merge_command_env(&config_env, HashMap::new());
...
fn merge_profiles_local_overrides_global() {
    ...
    let merged = {
        ...
        vrunner::config::schema::ProfilesConfig { entries }
    };
