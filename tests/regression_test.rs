...
use vrunner::config::schema::{
    Config, ServerConfig, SecurityConfig, TlsConfig, VttyConfig,
    DisplayConfig, CommandLogConfig, DaemonConfig, ExitConfig, EnvironmentConfig,
};
...
fn test_config() -> Config {
    Config {
        ...
        daemon: DaemonConfig {
            enabled: false,
            stdout_file: "/tmp/vrunner-test.out".to_string(),
            stderr_file: "/tmp/vrunner-test.err".to_string(),
        },
        handles: vec![],
...
async fn regression_kill_removes_from_list() {
    ...
    assert!(manager.get(&id).is_none(), "killed command still in manager");
...
async fn regression_http_client_has_timeout() {
    ...
    let client = vrunner::cli::subcommands::http_client();
...
async fn regression_list_empty_after_all_killed() {
    ...
    assert_eq!(manager.list().len(), 0, "list should be empty after killing all");
}
...
async fn regression_concurrent_spawns() {
    ...
    let list = manager.list();
    assert_eq!(list.len(), 10, "concurrent spawns: expected 10, got {}", list.len());
    for id in &ids {
        let _ = manager.kill(id, None).await;
    }
}
...
async fn regression_send_keys_to_running_command() {
    ...
    assert!(result.is_ok(), "send_keys should succeed for running command");
...
fn regression_default_config_is_valid() {
    ...
    let errors: Vec<_> = issues.iter()
        ...
        .collect();
    assert!(errors.is_empty(), "default config has validation errors: {:?}", errors);
}
...
fn regression_profile_override_port() {
    ...
    server.port = 3000;
    let profile = vrunner::config::schema::PartialConfig {
        server: Some(server),
        ..Default::default()
    };
...
fn regression_merge_command_env() {
    use vrunner::config::merge::merge_command_env;
...
async fn regression_spawn_with_env_vars() {
    ...
    if let Some(handle) = manager.get(&id) {
        let plain = handle.vtty_plain().await;
        assert!(plain.contains("test_value"), "env var not passed: got '{}'", plain);
    }
    let _ = manager.kill(&id, None).await;
}
...
async fn regression_freeze_thaw_command() {
    ...
    assert!(freeze_result.is_ok(), "freeze should succeed: {:?}", freeze_result.err());

    sleep(Duration::from_millis(100)).await;

    let thaw_result = manager.thaw(&id);
    assert!(thaw_result.is_ok(), "thaw should succeed: {:?}", thaw_result.err());
...
fn regression_emulator_bracketed_paste_roundtrip() {
    let mut emu = vrunner::vtty::emulator::VttyEmulator::new(24, 80, 1000);
...
fn regression_emulator_alt_screen_preserves_main() {
    ...
    let buf = emu.snapshot();
    assert_eq!(buf.get(0, 0).unwrap().ch, 'M', "main buffer should be restored");
}
...
fn regression_emulator_cursor_position_after_scroll() {
    ...
    assert!(buf.scrollback.len() > 0, "scrollback should have entries after overflow");
}

#[test]
fn regression_buffer_resize_preserves_content() {
    use vrunner::vtty::cell::Cell;
    let mut buf = vrunner::vtty::buffer::Buffer::new(10, 5, 100);
