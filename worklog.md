...
Task ID: refactor-5
Agent: main
Task: Refactor 5 critical code complexity findings — 5 individual commits

Work Log:
- Analyzed all 75 Rust source files (16,892 lines) for function length, cyclomatic complexity, and file size
- Identified top 100 longest/most complex functions and 15 files exceeding healthy size thresholds
- Committed 5 refactoring commits, each with architecture comments, tests, clippy verification

## Commit 1: `47ba632` — display.rs: extract 14 inner functions from run_display_loop
- Moved 14 nested helper functions + 3 mouse event types to module level
- run_display_loop: 1,937L → 1,290L (647-line reduction)
- Added 26 unit tests (base64_encode, try_parse_mouse_event, build_cell_sgr)

## Commit 2: `9cc3750` — display.rs: deduplicate action dispatch in run_display_loop
- Extracted 3 duplicated code patterns into shared functions:
  - execute_context_menu_action() — kill/purge/copy_id/restart (was 40L × 2)
  - dispatch_action_effect() — keybinding action dispatch (was 80L × 2)
  - handle_spawn_command() — F12 spawn-from-display (was 25L × 2)
- Added CommandLoopResult and SpawnCommandResult enums for control flow

## Commit 3: `7b461b3` — spawner.rs: break spawn() into focused helpers
- spawn(): 431L → ~80L orchestrator
- Extracted: build_handle_registry(), spawn_pty_reader(), spawn_emulator_writer(),
  spawn_stdin_writer(), spawn_process_waiter()
- Added 7 unit tests (escape_bytes, ExitStatus)

## Commit 4: `98273d6` — cli: split subcommands.rs into per-command modules
- subcommands.rs: 1,348L → 35L re-export layer
- Created src/cli/commands/ with 8 modules: mod, common, spawn, stop, list, purge,
  resize, cert, config
- All public API preserved via re-exports

## Commit 5: `88e74d4` — vtty+main: decompose parse() and async_main()
- parser.rs parse(): 327L → ~30L state-machine dispatcher + 10 focused handlers
- main.rs async_main(): 292L → ~90L orchestrator + 3 helpers
