...
//!
//! Connects to a running vrunner instance's control socket, sends a
...
/// Connect to a running vrunner instance's control socket and send a command.
///
...
pub async fn send_command_to_path(
    ...
    if !socket_path.exists() {
        anyhow::bail!(
            "No control socket at {} — is vrunner instance running?",
            socket_path.display()
        );
    }
