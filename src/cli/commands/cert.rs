...
/// Load config, map to `CertificateEntry` list, and build a `CertificateStore`.
fn load_cert_store() -> Result<CertificateStore> {
    ...
    let entries: Vec<CertificateEntry> = cfg
        ...
        .collect();
    CertificateStore::load_or_generate(entries)
