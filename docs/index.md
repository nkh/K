...
A virtual terminal runner with two deployment modes: a local-only **vrl** binary using UDS IPC, and a remote-capable **vrunner** binary with an HTTP server, web admin dashboard, WebSocket streaming, TLS, and REST API. Both share the same core library — VTTY emulator, process manager, configuration, interactive display, handles, hooks, and daemon — so everything you learn for one transfers to the other.

## Overview

vrl is a project that ships two binaries, each backed by the same `vrl_core` library:

### vrl (default feature `vrl`)
...
| [Web Dashboard Guide](how-to-guides/web-dashboard.md) | Using the browser-based admin UI with vrunner |
| [API Usage Guide](how-to-guides/api-usage.md) | REST API endpoints and examples for vrunner |
| [TLS & Certificates](how-to-guides/certificates.md) | Configure HTTPS/TLS for remote access |
| [Remote Access with TLS](how-to-guides/remote-tls.md) | End-to-end guide for secure remote vrunner |
| [Dev Server Workflow](how-to-guides/dev-server.md) | Use vrunner as a persistent dev server |
...
| [CI Pipeline Integration](how-to-guides/ci-pipeline.md) | Use vrunner in automated CI/CD pipelines |
...
| [API Reference](api.md) | Complete REST API specification for vrunner |
...
| [Security Model](explanation/security-model.md) | Authentication, TLS, and access control for vrunner |
