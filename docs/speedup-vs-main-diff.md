...
The `speedup` branch strips vrc of its entire HTTP/WebSocket server stack (axum, TLS, auth, certificate management, web UI handlers) and replaces it with a lightweight **Unix Domain Socket (UDS)** IPC mechanism. This eliminates ~9,600 lines of code and 20 crate dependencies, dramatically reducing startup time from ~150ms to under 5ms.
...
| File | Lines | Purpose |
|------|-------|---------|
| `src/cli/commands/cat.rs` | 107 | HTTP-based `vrc cat` (reqwest to server) |
| `src/cli/commands/cert.rs` | 119 | Certificate management commands |
| `src/cli/commands/purge.rs` | 201 | HTTP-based `vrc purge` |
| `src/cli/commands/screenshot.rs` | 194 | PNG screenshot generation |
| `src/cli/commands/resize.rs` | 163 | HTTP-based `vrc resize` |
| `src/cli/commands/spawn.rs` | 162 | HTTP-based `vrc spawn` |
...
- `use vrc::web::auth::AuthManager` — Auth manager initialization
- `use vrc::web::server::start_server` — Server startup call
...
**Added:**
- `use vrc::ipc::server::spawn_control_server` — UDS control socket startup
- `use vrc::ipc::socket_path_for_pid` — Socket path computation
...
**Key design change:** In main branch, `main()` built a multi-threaded tokio runtime and started both the server and display loop. In speedup, it uses a lighter `current_thread` runtime. IPC subcommands get their own minimal runtime without starting a full vrc instance.
...
3. `run_ipc_command()` — Runs the IPC subcommand using the UDS client with a minimal tokio runtime. Does NOT start a full vrc instance.
...
**After:** Reads PID files from `~/.local/share/vrc/instances/`, checks `/proc/<pid>/comm` for liveness, much faster and no full process table scan.
...
Without an HTTP server, vrc no longer needs concurrent I/O across multiple tasks. The only background tasks are:
...
|---------|-------------------|---------------------|
| **List instances** | `GET /api/commands` or `vrc list` (sysinfo scan) | `vrc list` (PID file + `/proc` check) |
| **Send keystrokes** | `POST /api/commands/:id/keys` (HTTP) or WebSocket binary frame | `vrc keys <pid> <keys>` → `ControlCommand::SendKeys` via UDS |
| **Cat VTTY output** | `GET /api/commands/:id/vtty/text` | `vrc cat <pid>` → `ControlCommand::Cat` via UDS |
| **Spawn command** | `POST /api/commands` (HTTP JSON) | `vrc spawn-in <pid> -- <cmd> <args>` → `ControlCommand::Spawn` via UDS |
| **Kill command** | `DELETE /api/commands/:id` | Handled via UDS `ControlCommand::Kill` |
| **Freeze command** | `POST /api/commands/:id/freeze` | `vrc freeze <pid>` → `ControlCommand::Freeze` via UDS |
| **Thaw command** | `POST /api/commands/:id/thaw` | `vrc thaw <pid>` → `ControlCommand::Thaw` via UDS |
| **Resize VTTY** | `POST /api/commands/:id/resize` | `vrc resize <pid> --rows N --cols M` → `ControlCommand::Resize` via UDS |
| **Stop instance** | `POST /api/stop` (HTTP) | `vrc stop` → SIGTERM directly |
...
tls:
  enabled: false
  cert_file: "~/.config/vrc/cert.pem"
  key_file: "~/.config/vrc/key.pem"

certificates:
  directory: "~/.config/vrc/certs/"
  entries: []
...
- IPC command handler integration tests (spawn a vrc, send keys, verify output)
...
| **Command spawn with args** | Fully working | `vrc -- cmd args` |
...
| **Shell completions** | Fully working | `vrc completions bash/zsh/fish/...` |
...
|---------|--------|-------------|
| **Web admin UI** | Entire HTTP server removed | CLI commands (`vrc list`, `vrc cat`, etc.) |
...
| **PNG screenshot generation** | `image` + `fontdue` crates removed | `vrc cat <pid>` for text output |
...
| **Client-mode discovery** | No HTTP probe on port 9090 | `vrc list` to find running instances |
| **`--bind` / `--port` flags** | No TCP server | N/A |
| **`--tls` flag** | No TLS | N/A |
| **`--auth` flag** | No authentication | N/A |
| **`vrc screenshot` command** | PNG code removed | `vrc cat <pid>` |
| **`vrc cert` command** | Certificate management removed | N/A |
