...
Learn how to set up secure remote access to your vrw instance — from a quick local test to a production-grade deployment with custom certificates and reverse proxies.
...
```bash
sudo certbot certonly --standalone -d vrw.example.com
# Certificates saved to:
#   /etc/letsencrypt/live/vrw.example.com/fullchain.pem
#   /etc/letsencrypt/live/vrw.example.com/privkey.pem
```
...
### 3. Start vrw with Custom Certificates
...
```bash
curl -v https://vrw.example.com:8443/api/commands
```
...
```bash
curl https://vrw.example.com:8443/api/commands
...
Navigate to `https://vrw.example.com:8443/admin`. The dashboard works identically over TLS — all traffic (including WebSocket) is encrypted.

### WebSocket

```javascript
const ws = new WebSocket('wss://vrw.example.com:8443/ws');
ws.onmessage = (event) => console.log(JSON.parse(event.data));
```

## Reverse Proxy with nginx

Place nginx in front of vrw for load balancing, rate limiting, and easier certificate management:
...
Start vrw bound to localhost only (nginx handles external TLS):

```bash
vrw --port 8080 --web --daemon
```
...
Start vrw without TLS (Caddy terminates it):
...
```bash
# Team A certificate
vrw cert generate --name team-a --commands "frontend,backend"
# Outputs: team-a-cert.pem, team-a-key.pem, team-a-token

# Team B certificate
vrw cert generate --name team-b --commands "database,migrations"
