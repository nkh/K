...
Learn how to run and monitor multiple production services from a single vrunner instance with TLS, per-command certificates, and team-based access control.
...
vrunner provides a unified dashboard and API for all of them, with TLS encryption and certificate-based access control for team security.
...
```bash
sudo vrunner --config /etc/vrunner/production.yaml --profile production
```
...
vrunner cert generate --name team-backend --commands "api,worker"

# DBA team — access to migrations
vrunner cert generate --name team-dba --commands "migrations"

# Ops team — access to health-check (and read-only on api for dashboards)
vrunner cert generate --name team-ops --commands "health-check,api"
```
...
They see only `api` and `worker`. In the browser, they configure the certificate in the TLS client settings and open `https://vrunner.example.com:8443/admin`.
...
```
https://vrunner.example.com:8443/admin
```
...
API_ID=$(curl -sk --cert team-backend-cert.pem --key team-backend-key.pem \
  https://vrunner.example.com:8443/api/commands | jq -r '.[] | select(.name=="api") | .id')

curl -sk --cert team-backend-cert.pem --key team-backend-key.pem \
  -X POST "https://vrunner.example.com:8443/api/commands/$API_ID/input" \
...
curl -sk --cert team-backend-cert.pem --key team-backend-key.pem \
  "https://vrunner.example.com:8443/api/commands/$API_ID/logs?limit=100"

# Search for errors
curl -sk --cert team-backend-cert.pem --key team-backend-key.pem \
  "https://vrunner.example.com:8443/api/commands/$API_ID/logs?search=ERROR"
```
...
server {
    ...
    ssl_client_certificate /etc/ssl/vrunner-ca.crt;
...
With `ssl_verify_client optional`, nginx passes the client certificate to vrunner, which performs the authorization check.
