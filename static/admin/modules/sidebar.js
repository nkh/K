    ...
    function renderCmdList(cmds, showServerBadge) {
        ...
        for (const { inst, cmd, cmdName } of cmds) {
            ...
            const serverBadge = showServerBadge
                ? `<span class="resource-badge" style="font-size:0.55rem;opacity:0.7;" title="${escHtml(inst.url)}">${escHtml(inst.label || inst.url)}</span>`
                : '';
